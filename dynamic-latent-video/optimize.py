...
def load_video(path, resize_width=320, max_frames=None):
    cap = cv2.VideoCapture(path); assert cap.isOpened(), f"open {path} failed"
...
def save_video(tensor_T_H_W_3, path, fps):
    ...
    for t in range(T):
        bgr = cv2.cvtColor((tensor_T_H_W_3[t].clamp(0,1).cpu().numpy()*255).astype(np.uint8), cv2.COLOR_RGB2BGR)
        vw.write(bgr)
    vw.release()
...
def init_siren_(lin, first=False, w0=30.0, c=6.0):
    with torch.no_grad():
        if first:
            ...
            lin.weight.uniform_(-math.sqrt(c/lin.in_features)/w0,
...
class SIRENDecoder(nn.Module):
    ...
    def __init__(self, dim_z=64, hidden=128, layers=5, w0=30.0):
        super().__init__()
        self.w0 = w0
        self.first = nn.Linear(2, hidden); init_siren_(self.first, first=True)
        self.hiddens = nn.ModuleList([nn.Linear(hidden, hidden) for _ in range(layers-2)])
        for h in self.hiddens: init_siren_(h, w0=w0)
        self.to_rgb = nn.Linear(hidden, 3); nn.init.xavier_uniform_(self.to_rgb.weight)

        # FiLM from latent z: produce per-layer scale/shift
        self.affines = nn.ModuleList([nn.Linear(dim_z, 2*hidden) for _ in range(layers-1)])
        for a in self.affines: nn.init.xavier_uniform_(a.weight)
        self.act = Sine(w0)

    def forward(self, coords, z):
        if z.dim()==1: z = z[None,:]
        B = coords.shape[0]
        x = self.first(coords); gamma,beta = self.affines[0](z).chunk(2, -1)
        x = self.act(x*gamma + beta)
        for i,layer in enumerate(self.hiddens, start=1):
            x = layer(x); gamma,beta = self.affines[i](z).chunk(2,-1)
            x = self.act(x*gamma + beta)
...
class PortHamiltonianDynamics(nn.Module):
    def __init__(self, dim=64, hidden=128, dt=0.1, energy_target=1.0):
        ...
        self.Hnet = nn.Sequential(nn.Linear(dim, hidden), nn.SiLU(),
                                  nn.Linear(hidden, hidden), nn.SiLU(),
                                  nn.Linear(hidden, 1))
        self.Dnet = nn.Sequential(nn.Linear(dim, hidden), nn.SiLU(),
                                  nn.Linear(hidden, hidden), nn.SiLU(),
                                  nn.Linear(hidden, 1))
        # Skew-symmetric field J(z): produce A(z), take A - A^T, apply to grad H
        self.Jgen = nn.Sequential(nn.Linear(dim, hidden), nn.SiLU(),
                                  nn.Linear(hidden, dim*dim))
        # Dissipation R(z): produce B(z), use R = B B^T (PSD)
        self.Bgen = nn.Sequential(nn.Linear(dim, hidden), nn.SiLU(),
                                  nn.Linear(hidden, dim*dim))

        # tiny control input u(z) to regulate energy around target
        self.unet = nn.Sequential(nn.Linear(dim, hidden), nn.Tanh(),
                                  nn.Linear(hidden, dim))
        self.energy_target = nn.Parameter(torch.tensor(energy_target), requires_grad=False)
...
class LatentVideo(nn.Module):
    ...
    def decode_frames(self, zs, chunk=65536, device=None):
        T,D = zs.shape; H,W = self.H, self.W
...
def train(mp4_path, iters=4000, resize_width=80, latent_dim=64,
    ...
    for it in range(1, iters+1):
        ...
        if it % 100 == 0:
            print(f"[{it:05d}] total={loss.item():.6f} rec={loss_rec.item():.6f} jerk={loss_jerk.item():.6f}")
...
def render_model(model, fps, T_train, T_extra=120, out_path="reconstruction.mp4"):
    ...
    print("wrote:", out_path.replace(".mp4","_fit.mp4"), "and", out_path.replace(".mp4","_extrap.mp4"))
...
if __name__ == "__main__":
    ...
    model, fps, T = train(args.video, iters=args.iters, resize_width=args.resize_width,
                          latent_dim=args.latent_dim, offset_n=args.offset_n, device=args.device)
